/* ==========================================================================
   Town Square Dentistry — live Google rating & reviews widget
   ...
   is missing or the API call fails for any reason, the page keeps the real,
   verified static reviews already in the markup instead of showing broken
   or fake data.
   ========================================================================== */
(function () {
  'use strict';
  ...
  var CONFIG = {
    ...
    PLACES_API_KEY: '',
    PLACE_QUERY: 'Town Square Dentistry, 207 SE 23rd Ave, Boynton Beach, FL 33435',
    MAX_REVIEWS: 3,
    CACHE_MINUTES: 60
  };
  ...
  function fail() {
    // No API key set (or the live call failed): the page already shows a
    // real, verified rating/review count baked into the markup (see
    // README.md), so there's nothing broken to hide — just leave the
    // status line as its static, honest default.
  }

  function fromCache() {
    try {
      var raw = sessionStorage.getItem('gmb-place-cache-townsquare');
      if (!raw) return null;
  ...
  function toCache(place) {
    try { sessionStorage.setItem('gmb-place-cache-townsquare', JSON.stringify({ ts: Date.now(), place: place })); } catch (e) { /* private mode etc — skip caching */ }
  }
